...
export function getApiKey(explicitKey?: string): string {
  ...
  if (cliConfig?.api_key) return cliConfig.api_key;
  throw new Error(
    "No API key found. Set BLAND_API_KEY environment variable or run `bland auth login`."
  );
}

export function getBaseUrl(): string {
  if (process.env.BLAND_BASE_URL) return process.env.BLAND_BASE_URL;
  const cliConfig = readBlandCliConfig();
  return cliConfig?.base_url || DEFAULT_BASE_URL;
}

export function isAuthenticated(explicitKey?: string): boolean {
  try {
